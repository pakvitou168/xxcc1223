  ...
  <div>
    <form @submit.prevent="handleSubmit">
      ...
      <div v-if="canEditCommissionData" class="text-right">
        <router-link :to="{name: 'PolicyIndex'}" class="btn btn-danger w-24 mr-2" tag="button">Cancel</router-link>
...
export default {
  ...
  methods: {
    handleSubmit() {
      axios.post(`/hs/policy-services/update-commission-data/${this.id}`, {
        ...
        .then(response => {
          ...
          notify( err.data?.message || this.SUCCESS_MESSAGE, "error", "bottom-right");
      })
    },
    handleIssueDate() {
