<template>
    <div>
        ...
        <div v-else v-for="(commission, index) in formValues" :key="index" class="pt-5 pb-2 border-b border-gray-300 first:pt-0 last:border-b-0">
            <ComissionItem
                :id="id"
                :index="index"
...
export default {
    ...
    methods: {
        checkCommissionDataAvailability(){
            axios.get(`/policy-service/is-commission-data-available/${this.id}`)
                .then(response => {
                    if(response.data)
        ...
        getCommissionData() {
            if (this.id)
                this.isLoading = true
                axios.get(`/policy-service/get-commission-data/${this.id}`)
                    .then(response => {
                        this.isLoading = false
                        this.formValues = response.data
                    })
                    .then(() => this.getBusinessNameByBusinessCode(this.formValues[0].business_code))
        },
