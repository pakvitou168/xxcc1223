  ...
  <div>
    <div class="intro-y flex items-center mt-8">
      ...
      <button class="btn btn-primary shadow-md mr-2 leading-6"
              ...
              @click="updateSubmitStatus('SBM')">Submit
      </button>
    </div>
        ...
        <div class="p-5">
          <div class="tab-content">
            <div id="config" class="tab-pane" :class="{ active: activeTab === 'config' }" role="tabpanel">
              <ConfigTab :id="id" :dataId="dataId" @checkPolicyStatus='isPolicyConfigurationCompleted'/>
            </div>
...
<script>
import ConfigTab from '../FormTabs/HS/Config.vue'
import CommissionTab from '../FormTabs/HS/Commission/Commission.vue'
import ReinsuranceTab from '../FormTabs/HS/Reinsurance/Reinsurance.vue'
...
export default {
  ...
  methods: {
    ...
    exportReinsuranceExcel() {
      location.href = '/hs/policy-services/export-reinsurance/' + this.id + '/' + this.formValues.product_code;
    },

    isPolicyReinsuranceCompleted() {
      if (this.id)
        axios.get(`/hs/policy-services/is-policy-reinsurance-completed/${this.id}`).then(response => {
          this.isReinsuranceCompleted = response.data;
          // Check if Policy Configuration is completed
          if (this.isReinsuranceCompleted) {
            axios.get(`/hs/policy-services/is-policy-configuration-completed/${this.id}`).then(response => {
    ...
    updateSubmitStatus(status) {
      axios.put(`/hs/policy-services/update-submit-status/${this.id}`, {status: status}).then(response => {
        if (response.data)
