<template>
  <div class="p-4  border-l border-r border-b border-gray-200 ">
    <div v-if="editable" class="grid grid-cols-5 gap-x-2 items-center">
        <div >
          <label class=" block mb-1" for="">Partner Group  <span class="text-red-600">*</span></label>
          <Dropdown
              v-model="formData.partner_group"
        ...
        <div >
          <label class=" block mb-1" for="">Participant  <span class="text-red-600">*</span></label>
          <Dropdown
      ...
      <div >
        <label for="share" class=" block mb-1">Share (%)  <span class="text-red-600">*</span></label>
        <InputNumber
      ...
      <div >
        <label for="ri_commission" class=" block mb-1">RI Commission (%)  <span class="text-red-600">*</span></label>
        <InputNumber
      ...
      <div class="relative">
        <label for="tax_fee" class=" block mb-1">Tax & Fee (%)  <span class="text-red-600">*</span></label>
        <div class="flex items-center p-0">
          <InputNumber
              v-model="formData.tax_fee"
              id="tax_fee"
              class="w-full p-0"
              placeholder="Tax & Fee (%)"
              :minFractionDigits="1"
          ...
          <div class="ml-2 flex items-center align-middle">
            <a @click="removeRow" class=" hover:bg-gray-100 rounded-full cursor-pointer" @mouseover="hover = true" @mouseleave="hover = false">
              <svg v-if="hover" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="red" viewBox="0 0 24 24" stroke="white" stroke-width="2">
    ...
    <div v-else class="grid grid-cols-10 gap-x-2 auto-cols-min">
        ...
        <div :class="[{'': !isChild}]" class="self-center">
            <div class="p-2 rounded border border-gray-300">{{ reinsuranceDetail.premium || 0 }}</div>
        </div>
        <div :class="[{'': !isChild}]" class="self-center">
        ...
        <div :class="[{'': !isChild}]" class="self-center">
            <div class="p-2 rounded border border-gray-300">{{ reinsuranceDetail.ri_commission_amt || 0   }}</div>
        </div>

        <div :class="[{'': !isChild}]" class="self-center">
            <div v-if="isChild" class="p-2 rounded border border-gray-300">{{ reinsuranceDetail.tax_fee || 0 }}</div>
        ...
        <div :class="[{'': !isChild}]" class="self-center">
            <div class="p-2 rounded border border-gray-300">{{ reinsuranceDetail.tax_fee_amt || 0   }}</div>
        </div>
...
export default {
  ...
  computed: {
    formattedPartnerGroupOptions() {
      return this.partnerGroupOptions
    },
    hasChild() {
