...
const canEdit = computed(() => {
    return hasPermission('HS_CLAIM_PAYMENT', 'UPDATE') && !row.value?.approved_status;
});
const canDelete = computed(() => {
    return hasPermission('HS_CLAIM_PAYMENT', 'DELETE') && row.value?.status === 'ACT' && row.value?.approved_status === 'PND' && !row.value?.accepted_status;
})
