    ...
    >
      <div class="grid grid-cols-1 gap-5">
        ...
        <div>
          ...
          <Textarea class="w-full" rows="3" v-model="form.comment"></Textarea>
          <span v-if="errors['comment']" class="text-xs text-red-500">{{ errors['comment'] }}</span>
        </div>
      </div>
        <template #footer>
            <Button label="Cancel" severity="danger" outlined @click="hideDialog"/>
            <Button label="Confirm" class="p-button-info px-2 py-2 border border-blue-500 text-white hover:bg-blue-600 bg-blue-500" :loading="submitted" autofocus @click="confirm" />
        </template>
    </Dialog>
</template>
...
export default {
    ...
    computed: {
        errorReason() {
            return (this.submitted && !this.form.comment) ? 'Reason is required.' : null
        },
    },
    methods: {
        confirm() {
          ...
          if (!this.form.comment) {
            this.errors.comment = 'Reason is required'
            return false
          }
          this.errors = []
