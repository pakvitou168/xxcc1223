        ...
        <div class="grid grid-cols-5 gap-x-2 gap-y-2 ml-6" v-if="updatedValue.schema_type == 'STANDARD'">
            <div class="col-span-2 grid grid-cols-2 gap-x-2 gap-y-2" v-if="updatedValue.max_number_of_day">
                <div>
                  ...
                  <Calendar  placeholder="" v-model="updatedValue.admission_date"  @change="changeDate" dateFormat="dd-M-yy" />
                  <span v-if="addmissionDateErr" class="text-error">
                ...
                <div class="">
                  ...
                  <InputNumber
                      ...
                      @change="changeDay"
                  />
                  <span v-if="daysErr" class="text-error">
                    {{daysErr[0]}}
                  </span>
                </div>
                <div>
                  <label for="from_date" class="block mb-1 font-bold">Discharge Date</label>
                  <Calendar  placeholder="" v-model="updatedValue.discharge_date"  @change="changeDate" dateFormat="dd-M-yy" />
                  <span v-if="dischargeDateErr" class="text-error">
                    {{dischargeDateErr[0]}}
                  </span>
                </div>
            </div>
    ...
    methods: {
        ...
        changeActualExpense(event) {
            ...
            if (actualExpense) {
                ...
                let claimableAmount = (maxClaimable >= actualExpense ? actualExpense : maxClaimable);
                let nonePayableExpense = parseFloat(actualExpense.replace(/,/g, ""))  - claimableAmount;
                console.log(
                    actualExpense,
                    claimableAmount
                )
