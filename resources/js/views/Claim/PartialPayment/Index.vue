  ...
  data() {
    return {
      addNew: {
        ...
        prefix: 'New',
        linkAddNew: '/claim/auto/partial-payments/new',
      },
      canAddNew: hasPermission('CLAIM_PARTIAL_PAYMENT', 'NEW'),
      tabulator: null,
      options: {
        ajaxURL: '/claim-partial-payments',
        columns: [
          {
            title: 'Risk Type',
          ...
          },
          {
            ...
            mutator: (_, row) => getActionButtons(
              hasPermission('CLAIM_PARTIAL_PAYMENT', 'VIEW'),
            ...
            cellClick: (e, cell) => {
              ...
              if (cell._cell.element.querySelector('a.edit svg')?.contains(e.target)) {
                this.$router.push({ name: 'PartialPaymentEdit', params: { id: dataId } })
                ...
                this.$router.push({ name: 'PartialPaymentDetail', params: { id: dataId } })
  ...
  methods: {
    ...
    handleRevise(id) {
      this.$confirm.require({
        ...
        rejectLabel: 'Cancel',
        accept: () => {
          PartialPaymentService.revise(id).then(res => {
            if (res.data?.success) {
              notify(res.data?.message,'success');
              this.$router.push({ name: 'PartialPaymentEdit', params: { id: id } });
            }
          }).catch(err => {
            console.log(err)
            notify(err?.response?.data?.message,'error');
    ...
    canRevise(approve_status) {
      let canRevisePermission = hasPermission('CLAIM_PARTIAL_PAYMENT', 'REVISE')
