<template>
  <div class="intro-y box p-5 mt-5">
    <div class="intro-y flex mb-4 p-1">
      <h2 class="text-xl font-medium mr-auto">Reinsurance Type Detail</h2>
      <button
        v-if="canDelete"
      ...
      <router-link
        v-if="canUpdate"
        :to="{ name: 'ReinsuranceTypeUpdate', params: { id: id } }"
...
export default {
  ...
  methods: {
    getReinsurance() {
      if (this.id) {
        axios.get(`/reinsurance-types/${this.id}`)
        .then((response) => {
    ...
    handleDelete(id) {
      ...
      blockScroll: false,
      accept: () => {
        axios.delete(`/reinsurance-types/${id}`)
        .then(response => {
          if (response.data.success) {
          notify(response.data.message, 'success', 'bottom-right');
          this.$router.push({ name: 'ReinsuranceTypeIndex' });
          } else {
          notify(response.data.message || 'Delete failed', 'error', 'bottom-right');
          }
        })
        .catch(error => {
          const message = error.response?.data?.message || 'Error deleting record';
