<template>
  <div class="intro-y box p-5 mt-5">
    <div class="intro-y flex mb-4 p-1">
      <h2 class="text-xl font-medium mr-auto">Reinsurance Partner Group Detail</h2>
      <button
        v-if="canDelete"
      ...
      <router-link
        v-if="canUpdate"
        :to="{ name: 'ReinsurancePartnerGroupUpdate', params: { id: id } }"
...
export default {
  ...
  methods: {
    getReinsuranceGroup() {
      if (this.id) {
        axios.get(`/reinsurance-partner-groups/${this.id}`).then((response) => {
    ...
    handleDelete(id) {
      ...
      blockScroll: false,
      accept: () => {
        axios.delete(`/reinsurance-partner-groups/${id}`)
        .then(response => {
