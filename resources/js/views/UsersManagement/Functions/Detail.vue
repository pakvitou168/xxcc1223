<template>
    <div class="intro-y box p-5 mt-5">
        <div class="intro-y flex mb-4 p-1">
            <h2 class="text-xl font-medium mr-auto">Function Detail</h2>
            ...
            <router-link v-if="canUpdate" :to="{ name: 'FunctionUpdate', params: { id: id }}" class="btn btn-primary mx-1 intro-x">
        ...
        <hr>
        <div class="flex m-2">
            <span class="text-base w-1/4 intro-x">Permissions</span>
            <p class="text-base text-bold intro-x">{{ permissions }}</p>
        </div>
        <div class="text-right mt-5">
            <router-link :to="{ name: 'FunctionIndex' }" class="btn btn-primary w-24 mr-1" tag="button">Back</router-link>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        permissions() {
            return this.formValues.permission?.join(', ')
        }
    },
    ...
    methods: {
        ...
        handleDelete(id) {
            this.$confirm.require({
                ...
                blockScroll: false,
                accept: () => {
                    axios.delete(`/functions/${id}`).then(response => {
