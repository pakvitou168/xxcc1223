...
const canEdit = computed(() => {
    return hasPermission('PA_QUOTATION', 'UPDATE') && row.value?.status === 'ACT' && row.value?.approved_status === 'PND' && !row.value?.accepted_status;
});
const canDelete = computed(() => {
    return hasPermission('PA_QUOTATION', 'DELETE') && row.value?.status === 'ACT' && row.value?.approved_status === 'PND' && !row.value?.accepted_status;
})
