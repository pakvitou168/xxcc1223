...
const canEdit = computed(() => {
    return hasPermission('AUTO', 'UPDATE') && row.value?.status === 'ACT' && row.value?.quotation?.approved_status === 'PND' && !row.value?.quotation?.accepted_status;
});
const canDelete = computed(() => {
    return hasPermission('AUTO', 'DELETE') && row.value?.status === 'ACT' && row.value?.quotation?.approved_status === 'PND' && !row.value?.quotation?.accepted_status;
})
