...
import groupService from '@/services/sm/group.service';
...
const title = computed(() => {
    return route.params.id ? `Group : ${formValue.value?.name}` : 'New Group'
});
const initForm = {
    code: '',
    name: '',
...
const loadDetail = async (refId) => {
    groupService.detail(refId).then(res => {
        formValue.value = res.data?.group
        defaultFormValue.value = JSON.parse(JSON.stringify(res.data?.group))
...
const handleSubmit = () => {
    ...
    saving.value = true
    if (id.value) {
        groupService.update(formValue.value, id.value).then(res => {
            emit('success', 'update')
        ...
        groupService.save(formValue.value).then(res => {
            notify(res.data.message, 'success')
            emit('success')
        }).catch(err => {
            if (err.status === 422) errors.value = err.response.data.errors
...
const loadRole = async () => {
    await groupService.role().then(res => {
        selections.roles = res.data
    })
}
const handlePageUrl = () => {
