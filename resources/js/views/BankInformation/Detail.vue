            ...
            <div class="overflow-x-auto scrollbar-hidden">
                <div class="w-full mb-4 table">
                    <div class="intro-y flex mb-4 p-1">
                        <h2 class="text-xl font-medium mr-auto">Bank Information Detail</h2>
                        ...
                        <router-link v-if="canUpdate" :to="{ name: 'BankInformationUpdate', params: { id: id }}">
                    ...
                    <hr>
                    <div class="flex m-2">
                        <span class="text-base w-1/4 intro-x">Account No.</span>
                        <p class="text-base text-bold intro-x">{{ formValues.account_no }}</p>
                    </div>
                    <hr>
                    <div class="flex m-2">
                        <span class="text-base w-1/4 intro-x">Account Name</span>
                        <p class="text-base text-bold intro-x">{{ formValues.account_name }}</p>
                    </div>
                    <hr>
                    <div class="flex m-2">
                        <span class="text-base w-1/4 intro-x">Currency</span>
                        <p class="text-base text-bold intro-x">{{ formValues.ccy }}</p>
                    </div>
                    <hr>
                    <div class="flex m-2">
                        <span class="text-base w-1/4 intro-x">Default</span>
                        <svg v-if="formValues.default" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" aria-labelledby="check-circle" role="presentation" class="text-green-500 fill-current text-success">
                    ...
                    <div class="text-right mt-5">
                        <router-link to="/bank-informations" class="btn btn-primary w-24 mr-1" tag="button">Back</router-link>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        getBankInformation() {
            if (this.id) {
                axios.get(`/api/bank-informations/${this.id}`)
                    .then(response => {
        ...
        handleDelete(id) {
            ...
            accept: () => {
                ...
                if (response.data.success) {
                    ...
                    this.$router.push({ name: "BankInformationIndex" })
                }
                }).catch(err => {
