<template>
    <div class="intro-y box p-5 mt-5">
        <div class="intro-y flex mb-4 p-1">
            <h2 class="text-xl font-medium mr-auto">Policy Wording Version Detail</h2>
            ...
            <router-link v-if="canUpdate" :to="{ name: 'PolicyWordingVersionEdit', params: { id: id }}" class="btn btn-primary mx-1 intro-x">
        ...
        <hr>
        <div class="flex m-2">
            <span class="text-base w-1/4 intro-x">Policy Wording</span>
            <p class="text-base text-bold intro-x">{{ formValues.policy_wording }}</p>
        </div>
        <hr>
        <div class="flex m-2">
            <span class="text-base w-1/4 intro-x">Year</span>
            <p class="text-base text-bold intro-x">{{ formValues.year }}</p>
        </div>
        <hr>
        <div class="flex m-2">
            <span class="text-base w-1/4 intro-x">Is Default</span>
            <svg v-if="formValues.is_default" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" aria-labelledby="check-circle" role="presentation" class="text-green-500 fill-current text-success">
...
export default {
    ...
    methods: {
        getData(){
            axios.get('/policy-wording-versions/' + this.id).then(response => {
