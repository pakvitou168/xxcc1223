<template>
    <div class="intro-y box p-5 mt-5">
        <div class="intro-y flex mb-4 p-1">
            <h2 class="text-xl font-medium mr-auto">Vehicle Classification Detail</h2>
            ...
            <router-link v-if="canUpdate" :to="{ name: 'VehicleClassificationUpdate', params: { id: id }}" class="btn btn-primary mx-1 intro-x">
        ...
        <hr>
        <div class="flex m-2">
            <span class="text-base w-1/4 intro-x">Surcharge (%)</span>
            <p class="text-base text-bold intro-x">{{ formValues.surcharge }}</p>
        </div>
        <div class="text-right mt-5">
            <router-link :to="{ name: 'VehicleClassificationIndex' }" class="btn btn-primary w-24 mr-1" tag="button">Back</router-link>
        </div>
    </div>
</template>
                ...
                accept: () => {
                    axios.delete(`/vehicle-classifications/${id}`).then(response => {
                        if (response.data.success) {
                            ...
                            this.$router.push({ name: 'VehicleClassificationIndex' });
                        }
                    }).catch(err => {
