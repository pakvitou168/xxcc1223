<template>
    <div class="intro-y box p-5 mt-5">
        <div class="intro-y flex mb-4 p-1">
            <h2 class="text-xl font-medium mr-auto">Model Detail</h2>
            ...
            <router-link  v-if="canUpdate" :to="{ name: 'ModelUpdate', params: { id: id }}" class="btn btn-primary mx-1 intro-x">
        ...
        <div class="flex m-2">
            ...
            <p class="text-base text-bold intro-x">{{ formValues.model }}</p>
        </div>
        <hr>
        <div class="flex m-2">
        ...
        <hr>
        <div class="flex m-2">
            <span class="text-base w-1/4 intro-x">Vehicle Type</span>
            <p class="text-base text-bold intro-x">{{ formValues.vehicle_type }}</p>
        </div>
        <hr>
        <div class="flex m-2">
            <span class="text-base w-1/4 intro-x">Classification</span>
            <p class="text-base text-bold intro-x">{{ formValues.classification }}</p>
        </div>
        <hr>
        <div class="flex m-2">
        ...
        <div class="text-right mt-5">
            <router-link to="/product-configuration/models" class="btn btn-primary w-24 mr-1" tag="button">Back</router-link>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        getModel(){
            axios.get('/models/' + this.id).then(response => {
        ...
        handleDelete(id) {
            this.$confirm.require({
                ...
                blockScroll: false,
                accept: () => {
                    axios.delete(`/models/${id}`).then(response => {
