  ...
  data() {
    return {
      ...
      addNew: {
        ...
        name: "Model",
        prefix: "Add New",
        linkAddNew: "/product-configuration/models/new",
      },
      tabulator: null,
      options: {
        persistenceID: "models-table",
        ajaxURL: "/models",
        columns: [
          {
            title: 'ID',
          ...
          },
          {
            ...
            cellClick: (e, cell)=>{
              ...
              if (cell._cell.element.querySelector("a.edit svg").contains(e.target)) {
                this.$router.push("/product-configuration/models/" + dataId + "/edit");
              } else if (cell._cell.element.querySelector("a.view svg").contains(e.target)) {
                this.$router.push("/product-configuration/models/" + dataId);
  ...
  methods: {
    ...
    setFilter(e) {
      this.tabulator.setFilter([
        {field: 'model', type: 'like', value: e.search},
        {field: 'vehicle_type', type: 'like', value: e.search},
        {field: 'classification', type: 'like', value: e.search},
        [
