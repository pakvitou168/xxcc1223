      ...
      <div class="overflow-x-auto scrollbar-hidden">
        <div class="w-full mb-4 table">
          <div class="intro-y flex mb-4 p-1">
              <h2 class="text-xl font-medium mr-auto">Third Party Detail</h2>
              ...
              <router-link v-if="canUpdate" :to="{ name: 'ThirdPartyEdit', params: { id: id }}">
          ...
          <hr>
          <div class="flex m-2">
              <span class="text-base w-1/4 intro-x">Driving License Number</span>
          ...
          <div class="flex m-2">
              ...
              <p class="text-base text-bold intro-x">{{ formValues.vehicle_make }}</p>
          </div>
          <hr>
          <div class="flex m-2">
              <span class="text-base w-1/4 intro-x">Model</span>
              <p class="text-base text-bold intro-x">{{ formValues.vehicle_model }}</p>
          </div>
          <hr>
          <div class="flex m-2">
              <span class="text-base w-1/4 intro-x">Plate No.</span>
              <p class="text-base text-bold intro-x">{{ formValues.plate_no }}</p>
          </div>
          <hr>
          <div class="flex m-2">
              <span class="text-base w-1/4 intro-x">Engine No.</span>
              <p class="text-base text-bold intro-x">{{ formValues.engine_no }}</p>
          </div>
          <hr>
          <div class="flex m-2">
              <span class="text-base w-1/4 intro-x">Manufacturing Year</span>
              <p class="text-base text-bold intro-x">{{ formValues.manufacturing_year }}</p>
          </div>
          <hr>
          <div class="flex m-2">
          ...
          <div class="text-right mt-5">
                <router-link :to="{name: 'ThirdPartyIndex'}" class="btn btn-primary w-24 mr-1" tag="button">Back</router-link>
          </div>
        </div>
      </div>
    </div>
...
export default {
  data() {
    return {
      ...
      formValues: {},
      canUpdate: hasPermission('THIRD_PARTY', 'UPDATE'),
      canDelete: hasPermission('THIRD_PARTY', 'DELETE'),
    }
  },
  computed: {
    getIssueDate() {
