      ...
      <div class="overflow-x-auto scrollbar-hidden">
        <div class="w-full mb-4 table">
          <div class="intro-y flex mb-4 p-1">
              <h2 class="text-xl font-medium mr-auto">Adjuster Company Detail</h2>
              ...
              <router-link v-if="canUpdate" :to="{ name: 'AdjusterCompanyEdit', params: { id: id }}">
          ...
          <div class="text-right mt-5">
                <router-link :to="{name: 'AdjusterCompanyIndex'}" class="btn btn-primary w-24 mr-1" tag="button">Back</router-link>
          </div>
        </div>
      </div>
    </div>
...
export default {
  data() {
    return {
      ...
      formValues: {},
      canUpdate: hasPermission('ADJUSTER_COMPANY', 'UPDATE'),
      canDelete: hasPermission('ADJUSTER_COMPANY', 'DELETE'),
    }
  },
  computed: {
    getIssueDate() {
  ...
  methods: {
    ...
    handleDelete(id) {
      this.$confirm.require({
        ...
        blockScroll: false,
        accept: () => {
          AdjusterService.delete(id).then(res => {
            if (res.data.success) {
