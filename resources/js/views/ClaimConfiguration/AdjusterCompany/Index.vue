...
import AdjusterCompanyService from '@/services/claim/adjuster_company.service'
...
export default {
  ...
  data() {
    return {
      addNew: {
        label: 'Adjuster Companies',
        prefix: 'New',
        linkAddNew: '/claim-configuration/adjuster-companies/new',
      },
      canAddNew: hasPermission('ADJUSTER_COMPANY', 'NEW'),
      tabulator: null,
      options: {
        ajaxURL: '/adjuster-companies',
        columns: [
          {
            title: 'Name (English)',
            field: 'name_en',
          ...
          },
          {
            ...
            mutator: () => getActionButtons(
              hasPermission('ADJUSTER_COMPANY', 'VIEW'),
              hasPermission('ADJUSTER_COMPANY', 'UPDATE'),
              hasPermission('ADJUSTER_COMPANY', 'DELETE'),
            ),
            cellClick:(e, cell)=> {
              let dataId = cell._cell.row.data.id;

              if (cell._cell.element.querySelector('a.edit svg')?.contains(e.target)) {
                this.$router.push({name: 'AdjusterCompanyEdit', params: {id: dataId}})
              } else if (cell._cell.element.querySelector('a.view svg')?.contains(e.target)) {
                this.$router.push({name: 'AdjusterCompanyDetail', params: {id: dataId}})
  ...
  methods: {
    handleDelete(id) {
      this.$confirm.require({
        ...
        blockScroll: false,
        accept: () => {
          AdjusterCompanyService.delete(id).then(res => {
            if (res.data.success) {
