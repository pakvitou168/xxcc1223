<template>
    <Dialog header="Upload Vehicles" ref="dl" class="p-fluid w-6/12 custom-dialog z-10" position="top"
        ...
        <template #footer>
            <div class="flex w-full">
                ...
                <Button label="Cancel" class="button-default mr-1" @click="hideDialog" />
                <Button label="Submit" :loading="isSaving"
                    :class="{ 'opacity-50 cursor-not-allowed': isDisabledSubmitButton,'button-info' : 1 }"
...
export default {
    ...
    methods: {
                    ...
                    this.getVehicleModel(rows).finally(() => {
                        this.checkVehicleError(rows).finally(() => {
                            if (this.vehicleUploadErrorIndexes.length > 0) {
                                ...
                                formData.append('master_data_type', 'QUOTATION')
        ...
        async checkVehicleError(rows) {
            console.log(rows, this.makeList)
            rows.map((item, index) => {
                var makeId = null;
                var modelId = null;
                ...
                for (let key in this.vehicleUsageOptions) {
                    if (item[13] == this.vehicleUsageOptions[key].label) {
                        vehicleUsage = this.vehicleUsageOptions[key].label;
                        break;
                    }
                }
                console.log(makeId, modelId, covers, vehicleUsage)
        ...
        toastMessage(msg, type, position = 'bottom') {
            this.$toast.add({ severity: type, summary: 'Alert', detail: msg, life: 3000, position: position });
        },
    },
