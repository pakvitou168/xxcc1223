<template>
    <div>
        <div class="intro-y flex flex-col sm:flex-row items-center">
            <div>
                <span v-if="isEndorsement && !isGeneralEndorsement" class="text-lg font-bold">Total Premium:
                    ...
                    <span v-else>{{ newPremium || newPremium == 0 ? newPremium : totalPremium }}</span>
                </span>
            ...
            <div class="w-full sm:w-auto flex ml-auto">
                <button v-if="canDeleteVehicle" class="button-danger shadow-md" @click="handleDelete">Delete</button>
                <button v-if="canOpenUploadDialog" class="button-primary shadow-md ml-2" @click="openFormUploadDialog">Upload vehicles</button>
                <button v-if="canAddVehicle" class="button-primary shadow-md ml-2" @click="openFormDialog">Add vehicle</button>
            </div>
        </div>
        <div class="mt-5">
        ...
        <portal to="destination">
            ...
            />
            <FormUploadDialog
                v-if="canOpenUploadDialog"
                :isVisible="isShownUploadDialog"
                :submitted="submitted"
                :productCode="productCode"
                :masterDataId="masterDataId"
                :quotationDocumentNo="quotationDocumentNo"
...
<script>
    ...
    import FormUploadDialog from './FormUploadDialog.vue'
	...
	export default {
        ...
        methods: {
            ...
            openDeleteVehicleQuotationOrPolicyDialog(){
                this.$confirm.require({
                    ...
                    accept: () => {
                        ...
                        this.startCalculatingPremium()
                        axios.post('/autos/functions/delete-vehicles', formData).then(response => {
                            if (response.data[0].stat == 'SUC') {
                                this.newPremium = response.data[0].total_premium ?? 0
                                this.toastMessage(response.data[0].message, 'Success')
                                this.tabulator.replaceData()
                            } else {
                        ...
                        .then(() => {
                            if(!this.quotationDocumentNo && this.isQuotation)
                                this.handleQuotationNumGeneration()
                            if(this.isPolicy){
            ...
            toastDeductibleMessage() {
                this.$toast.add({
                    severity: 'success',
                    detail: 'Deductibles have been generated to default',
                    life: 4000
                });
            },
