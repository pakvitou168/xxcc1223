...
@section('content')
    ...
    <body class="{{ $is_en ? 'font-candara':'font-khmer-os' }} px-20 my-0">
        @foreach ($cause_of_loss as $detail)
            <div class="break-page">
                ...
                <div class="avoid-break">
                    <div class="float-right mt-5 {{$is_en?'uppercase':''}}">
                        <p class="mb-2 white-space-nowrap">{{__('Date')}}: {{ $approved_at ? date('d/m/Y', strtotime($approved_at)) : '' }}</p>
                        <p class="mb-2 white-space-nowrap">{{__('Payment No')}}: {{ $detail['payment_no'] }}</p>
                        <p class="mb-2 white-space-nowrap">{{__('Account Code')}}: {{ $product_code }}</p>
                    </div>
                    <div style="border-bottom: 3px solid #000" class="w-full mb-5"></div>
                    <div class="mb-5 avoid-break">
                        <table class="w-full border-collapse">
                            <tr>
                                <td class="w-1/3 {{$is_en?'uppercase':''}}">{{__('Claim No')}}</td>
                                <td class="w-2/3">: {{ $detail['claim_no'] }}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="w-2/3">: {{__('Being Paid to')}} {{ $detail['payee_name'] }} {{__('Under Claim Number')}}
                                    {{ $detail['claim_no'] }}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="w-2/3">: {{ 'USD ' . number_format($detail['total_claim'], 2) }}</td>
                            </tr>
                        </table><br><br><br>
                        <table class="w-full border-collapse">
                        ...
                        {{-- <table class="w-full border-collapse">
                            <tr>
                                <td class="w-1/2">{{__('Requested by')}}: {{ $updated_by_name??'___________________________________' }}</td>
                                <td class="w-1/2">{{__('DEPARTMENT: Claim')}}</td>
                            </tr>
                        </table><br><br><br><br> --}}
                        @if ($detail['total_claim'] < 2000)
                            <div class="clearfix">
                            ...
                            @elseif($detail['total_claim'] < 50000)
                            <div class="clearfix">
