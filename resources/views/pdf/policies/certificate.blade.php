...
@section('content')
    <body class="m-0">
        @foreach ($auto_details as $auto_detail)
            ...
            <div class="break-page mx-10">
                ...
                <div class="clearfix" style="height: 100px;">
                    <div class="w-full inline-block text-10pt font-candara ml-6">
                        <p class="mt-8 w-full" style="height: 38px;">{{ $auto->insured_name }}</p>
                ...
                <div class="clearfix" style="height: 101px;">
                    <div class="w-3/5 inline-block float-right ml-2 text-10pt font-candara" style="margin-top: 3px;">
                        <p class="w-full mt-2.5 font-khmer-os" style="min-height: 17px;">- {{ $auto_detail->plate_no }}</p>
                        <p class="w-full mt-1.5" style="min-height: 17px;">- {{ optional($auto_detail->makeModel)->make->make }}/{{ optional($auto_detail->makeModel)->model }}</p>
                        <p class="w-full mt-1" style="min-height: 17px;">- {{ $auto_detail->chassis_no }}/{{ $auto_detail->engine_no }}</p>
                    </div>
                </div>
                ...
                <div class="clearfix" style="height: 60px;">
                    <div class="w-3/5 inline-block float-right ml-2 text-10pt font-candara">
                        <p class="w-full mt-5" style="min-height: 20px;">
                            - {{ $auto_detail->endos_day_remaining ?? $auto->effective_day }}
                            days from
                            {{ $policy->version ? \Carbon\Carbon::parse($auto_detail->endorsement_e_date)->format('d M Y') : \Carbon\Carbon::parse($auto->effective_date_from)->format('d F Y') }}
                            to {{ \Carbon\Carbon::parse($auto->effective_date_to)->format('d F Y') }}
                            (Both days inclusive)
                        </p>
                    </div>
                </div>
                ...
                <div class="clearfix" style="height: 67px;">
                    <div class="w-3/5 inline-block float-right ml-2 text-10pt font-candara">
                        <p class="w-full mt-7" style="min-height: 20px;">- {{ $auto_detail->covers }}</p>
                    </div>
                </div>
                ...
                <div class="clearfix" style="height: 65px;">
                    <div class="w-3/5 h-100 inline-block float-right ml-2 text-10pt font-candara">
                        @php
                            $deductibleCount = count($auto_detail->deductibles);
                        ...
                        @endphp
                        <p class="{{ $marginClass }}">
                            @foreach ($auto_detail->deductibles as $deductible)
                    ...
                    <div class="clearfix w-full">
                        <div class="w-3/5 inline-block float-right ml-2 text-10pt font-candara" style="margin-top: 138px;">
                            ...
                            <p class="w-full mt-2" style="height: 20px;">
                                {{ $policy->version ?  \Carbon\Carbon::parse($auto_detail->endorsement_e_date)->format('d M Y') : \Carbon\Carbon::parse($auto->effective_date_from)->format('d M Y')}}
                                To
                                {{ \Carbon\Carbon::parse($auto->effective_date_to)->format('d M Y') }}
                            </p>
                            <p class="w-full mt-0.5 font-candara font-khmer-os" style="height: 18px;font-size: 12px;">{{ $auto_detail->plate_no }}</p>
                            <p class="w-full mt-1 table" style="height: 22px;width: 200px;line-height: 0.95; font-size: 12px;">
                                <span class="table-cell align-bottom">{{ $auto_detail->chassis_no }}/{{ $auto_detail->engine_no }}</span>
                            </p>
                            <p class="w-full mt-1 table" style="height: 22px;width: 180px;line-height: 0.95;font-size: 12px;">
                                <span class="table-cell align-bottom">{{ $auto_detail->covers }}</span>
                            </p>
