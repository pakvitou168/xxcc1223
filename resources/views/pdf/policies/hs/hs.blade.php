...
<body class="{{ !$is_en ? 'font-khmer-os' : 'font-candara' }} mb-0">
    <div class="text-center">
        ...
        <div class="flex flex-col py-2">
            <div class="text-right">
                <div class="mt-1">{{__('Policy No')}} : <span>{{ $hs['policy_no'] }}</span></div>
    ...
    'productPolicyWording' => __('Product Policy Wording', [
        'productName' => !$is_en ?
            optional($hs['product'])->name_kh : optional($hs['product'])->name
    ])
            ...
            <table class="border-collapse w-full font-size-content">
                <thead style="display: table-row-group;">
                    <tr>
                        ...
                        @if($hs['standard_total_premium']?->plan_1 > 0)
                        <th class="p-1">{{__('Plan I (USD)')}}</th> @endif
                        @if($hs['standard_total_premium']?->plan_2 > 0)
                        <th class="p-1">{{__('Plan II (USD)')}}</th> @endif
                        @if($hs['standard_total_premium']?->plan_3 > 0)
                        <th class="p-1">{{__('Plan III (USD)')}}</th> @endif
                        @if($hs['standard_total_premium']?->plan_4 > 0)
                        <th class="p-1">{{__('Plan IV (USD)')}}</th> @endif
                        @if($hs['standard_total_premium']?->plan_5 > 0)
                    ...
                    <tr>
                        ...
                        @if($hs['standard_total_premium']?->plan_1 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_base_plan_amount']?->plan_1, 2) }}
                        </td>@endif
                        @if($hs['standard_total_premium']?->plan_2 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_base_plan_amount']?->plan_2, 2) }}
                        </td>@endif
                        @if($hs['standard_total_premium']?->plan_3 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_base_plan_amount']?->plan_3, 2) }}
                        </td>@endif
                        @if($hs['standard_total_premium']?->plan_4 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_base_plan_amount']?->plan_4, 2) }}
                        </td>@endif
                        @if($hs['standard_total_premium']?->plan_5 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_base_plan_amount']?->plan_5, 2) }}
                        </td>@endif
                    </tr>
                    @endif
                    <tr>
                        <td colspan="3" class="p-1 text-right">{{$is_en ? $hs['standard_premium_per_person']?->name :
            splitWordsKm($hs['standard_premium_per_person']?->name)}}</td>
                        @if($hs['standard_total_premium']?->plan_1 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_premium_per_person']?->plan_1, 2) }}
                        </td>@endif
                        @if($hs['standard_total_premium']?->plan_2 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_premium_per_person']?->plan_2, 2) }}
                        </td>@endif
                        @if($hs['standard_total_premium']?->plan_3 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_premium_per_person']?->plan_3, 2) }}
                        </td>@endif
                        @if($hs['standard_total_premium']?->plan_4 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_premium_per_person']?->plan_4, 2) }}
                        </td>@endif
                        @if($hs['standard_total_premium']?->plan_5 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_premium_per_person']?->plan_5, 2) }}
                        </td>@endif
                    </tr>
                    <tr>
                        <td colspan="3" class="p-1 text-right">{{$is_en ? $hs['standard_total_premium']?->name :
            splitWordsKm($hs['standard_total_premium']?->name) }}</td>
                        @if($hs['standard_total_premium']?->plan_1 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_total_premium']?->plan_1, 2) }}
                        </td> @endif
                        @if($hs['standard_total_premium']?->plan_2 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_total_premium']?->plan_2, 2) }}
                        </td> @endif
                        @if($hs['standard_total_premium']?->plan_3 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_total_premium']?->plan_3, 2) }}
                        </td> @endif
                        @if($hs['standard_total_premium']?->plan_4 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_total_premium']?->plan_4, 2) }}
                        </td> @endif
                        @if($hs['standard_total_premium']?->plan_5 > 0)
                            <td class="p-1 text-right">
                                {{number_format($hs['standard_total_premium']?->plan_5, 2) }}
                        </td> @endif
                    </tr>
                </tbody>
            </table>
            ...
            @if ($hs['optional_benefits']->count() >= 1)
                <table class="border-collapse w-full font-size-content" style="page-break-inside:auto">
                    ...
                    <tbody>
                        ...
                        @endforeach
                        <tr>
                            <td colspan="2" class="p-1 text-right">{{ $is_en ? $hs['optional_premium_per_person']?->name :
                    splitWordsKm($hs['optional_premium_per_person']?->name) }}</td>
                            @if($hs['optional_total_premium']?->plan_1)
                                <td class="p-1 text-right">{{
                            number_format($hs['optional_premium_per_person']?->plan_1, 2) }}</td>@endif
                            @if($hs['optional_total_premium']?->plan_2)
                                <td class="p-1 text-right">{{
                            number_format($hs['optional_premium_per_person']?->plan_2, 2) }}</td>@endif
                            @if($hs['optional_total_premium']?->plan_3)
                                <td class="p-1 text-right">{{
                            number_format($hs['optional_premium_per_person']?->plan_3, 2) }}</td>@endif
                            @if($hs['optional_total_premium']?->plan_4)
                                <td class="p-1 text-right">{{
                            number_format($hs['optional_premium_per_person']?->plan_4, 2) }}</td>@endif
                            @if($hs['optional_total_premium']?->plan_5)
                                <td class="p-1 text-right">{{
                            number_format($hs['optional_premium_per_person']?->plan_5, 2) }}</td>@endif
                        </tr>
                        <tr>
                            <td colspan="2" class="p-1 text-right">{{ $is_en ? $hs['optional_total_premium']?->name :
                    splitWordsKm($hs['optional_total_premium']?->name) }}</td>
                            @if($hs['optional_total_premium']?->plan_1)
                                <td class="p-1 text-right">{{
                            number_format($hs['optional_total_premium']?->plan_1, 2) }}</td>@endif
                            @if($hs['optional_total_premium']?->plan_2)
                                <td class="p-1 text-right">{{
                            number_format($hs['optional_total_premium']?->plan_2, 2) }}</td>@endif
                            @if($hs['optional_total_premium']?->plan_3)
                                <td class="p-1 text-right">{{
                            number_format($hs['optional_total_premium']?->plan_3, 2) }}</td>@endif
                            @if($hs['optional_total_premium']?->plan_4)
                                <td class="p-1 text-right">{{
                            number_format($hs['optional_total_premium']?->plan_4, 2) }}</td>@endif
                            @if($hs['optional_total_premium']?->plan_5)
                                <td class="p-1 text-right">{{
                            number_format($hs['optional_total_premium']?->plan_5, 2) }}</td>@endif
                        </tr>
                    </tbody>
                </table>
            @endif
    ...
    @endif
    @if($hs['total_additional_premium'] > 0)
        <div class="pt-1 pb-3 avoid-break">
            <div class="clearfix">
                <div class="w-1/3 inline-block float-left">
    ...
    @endif
    @if(
            $hs['total_premium']?->plan_1 || $hs['total_premium']?->plan_2 || $hs['total_premium']?->plan_3 ||
            $hs['total_premium']?->plan_4 || $hs['total_premium']?->plan_5
        )
                        ...
                        <table class="border-collapse w-full font-size-content">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @if($hs['standard_total_premium']?->plan_1 > 0 || $hs['optional_total_premium']?->plan_1 > 0)
                                        <td class="p-1 text-right">
                                            {{$hs['optional_total_premium']?->plan_1 > 0 ? number_format($hs['optional_premium_per_person']?->plan_1, 2) : '' }}
                                    </td>@endif
                                    @if($hs['standard_total_premium']?->plan_2 > 0 || $hs['optional_total_premium']?->plan_2 > 0)
                                        <td class="p-1 text-right">
                                            {{$hs['optional_total_premium']?->plan_2 > 0 ? number_format($hs['optional_premium_per_person']?->plan_2, 2) : '' }}
                                    </td>@endif
                                    @if($hs['standard_total_premium']?->plan_3 > 0 || $hs['optional_total_premium']?->plan_3 > 0)
                                        <td class="p-1 text-right">
                                            {{$hs['optional_total_premium']?->plan_3 > 0 ? number_format($hs['optional_premium_per_person']?->plan_3, 2) : '' }}
                                    </td>@endif
                                    @if($hs['standard_total_premium']?->plan_4 > 0 || $hs['optional_total_premium']?->plan_4 > 0)
                                        <td class="p-1 text-right">
                                            {{$hs['optional_total_premium']?->plan_4 > 0 ? number_format($hs['optional_premium_per_person']?->plan_4, 2) : '' }}
                                    </td>@endif
                                    @if($hs['standard_total_premium']?->plan_5 > 0 || $hs['optional_total_premium']?->plan_5 > 0)
                                        <td class="p-1 text-right">
                                            {{$hs['optional_total_premium']?->plan_5 > 0 ? number_format($hs['optional_premium_per_person']?->plan_5, 2) : '' }}
                                    </td>@endif
                                </tr>
                                @endif
                                <tr>
    ...
    @if($hs['warranty'] || $hs['warranty_kh'])
        <div class="clearfix avoid-break">
            ...
            <div class="w-2/3 inline-block float-right">
                <div class="mb-3 font-candara">{!! $is_en ? $hs['warranty'] : $hs['warranty_kh'] !!}</div>
            </div>
        </div>
    @endif
    @if($hs['memorandum'] || $hs['memorandum_kh'])
        <div class="clearfix avoid-break">
            ...
            <div class="w-2/3 inline-block float-right">
                <div class="mb-3 font-candara">{!! $is_en ? $hs['memorandum'] : $hs['memorandum_kh']  !!}</div>
            </div>
        </div>
    @endif
    @if($hs['subjectivity'] || $hs['subjectivity_kh'])
        <div class="clearfix avoid-break">
            ...
            <div class="w-2/3 inline-block float-right">
                <div class="mb-3 font-candara">{!! $is_en ? $hs['subjectivity'] : $hs['subjectivity_kh'] !!}</div>
            </div>
        </div>
    @endif
    @if($hs['remark'] || $hs['remark_kh'])
        <div class="clearfix avoid-break">
            ...
            <div class="w-2/3 inline-block float-right">
                <div class="mb-3 font-candara">{!! $is_en ? $hs['remark'] : $hs['remark_kh'] !!}</div>
            </div>
        </div>
    @endif
    <div class="clearfix">
            ...
            <div class="clearfix">
                <div class="w-1/3 inline-block float-left">
                    <div class="relative" style="min-height: 150px;">
                        @if(
                                request('noStamp') && !$hs['hasLetterHead'] && isset($hs['signature']) &&
                                file_exists(public_path($hs['signature']->file_url))
                            )
                                                    <img class="img-over" src="{{ public_path($hs['signature']->file_url) }}" alt=""
