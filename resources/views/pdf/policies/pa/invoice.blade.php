    ...
    <body class='font-candara'>
        ...
        <div class="clearfix">
            <div class="w-2/3 inline-block float-right">
                <div class="text-right">
                    <span class='font-khmer-os'>លេខរៀងវិក្កយបត្រ</span> - <span>Invoice No.</span> : {{ $invoice->invoice_no ?? '-' }}
                </div>
                <div class="text-right">
        ...
        <div class="clearfix margin-bottom-2">
            ...
            <div class="w-2/3 padding-top-6 inline-block font-bold float-right">
                {!! "&nbsp;" !!} {{ $invoice->customer_name ?? $data->insured_name ?? '-' }}
            </div>
        </div>
        ...
        <div class="clearfix margin-bottom-2">
            ...
            <div class="w-2/3 padding-top-6 inline-block float-right">
                {!! "&nbsp;" !!} {{ $invoice->address ?? $data->customer->address ?? '-' }}
            </div>
        </div>
        ...
        <div class="clearfix margin-bottom-2">
            ...
            <div class="w-2/3 padding-top-6 inline-block float-right">
                {!! "&nbsp;" !!} {{ $invoice->policy_document_no ?? $data->policy->document_no ?? '-' }}
            </div>
        </div>
        ...
        <div class="clearfix margin-bottom-2">
            ...
            <div class="w-2/3 padding-top-6 inline-block float-right">
                {!! "&nbsp;" !!} {{ $invoice->product_name ?? $data->product->name ?? 'Personal Accident' }}
            </div>
        </div>
        ...
        <div class="clearfix margin-bottom-2">
            ...
            <div class="w-2/3 padding-top-6 inline-block float-right">
                {!! "&nbsp;" !!} {{ $invoice->insurance_period ?? $data->insurance_period ?? '-' }}
            </div>
        </div>
        ...
        <div class="clearfix mb-1">
            <table class="w-full border border-black border-collapse font-size-table">
                ...
                <tr>
                    ...
                    @if(empty($invoice->total_premium))
                        <td class="text-center">-</td>
                    @else
                        <td class="font-bold text-center">{{ '$'.number_format($invoice->total_premium , 2) }}</td>
                    @endif
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    @if(empty($invoice->total_premium) || empty($invoice->exch_rate))
                        <td class="text-center">-</td>
                    @else
                        <td class="font-bold text-center">{{ number_format(round($invoice->total_premium * $invoice->exch_rate), 2) }} <span class='font-khmer-os'>រៀល</span></td>
                    @endif
                </tr>
                <tr>
                    <td class="padding-left-5 ">
