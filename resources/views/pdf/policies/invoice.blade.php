...
@section('content')
    <body class='font-candara'>
        <h3 class="font-size-title font-bold text-center"><span class='font-khmer-os-moul-light'>វិក្កយបត្រ</span> - <span>INVOICE</span></h3>
        ...
        <div class="clearfix">
            <div class="w-2/3 inline-block float-right">
                <div class="text-right">
                    <span class='font-khmer-os'>លេខរៀងវិក្កយបត្រ</span> - <span>Invoice No.</span> : {{ $invoice->inv_cdn_no ?? '-' }}
                </div>
                <div class="text-right">
                ...
                <div class="text-right">
                    <span class='font-khmer-os'>កូដ</span> - <span>Code</span> : {{ $invoice->code ?? '-' }}
                </div>
            </div>
        </div>
        ...
        <div class="clearfix margin-bottom-2">
            ...
            <div class="w-2/3 padding-top-6 inline-block float-right">
                {!! "&nbsp;" !!} {{ !empty($invoice->product_name) ? $invoice->product_name : '-' }}
            </div>
        </div>
        ...
        <div class="clearfix margin-bottom-2">
            ...
            <div class="w-2/3 padding-top-6 inline-block float-right">
                {!! "&nbsp;" !!}
                @if(empty($invoice->endorsement_e_date))
                    <span> - </span>
                @else
                    <span> {{ \Carbon\Carbon::parse($invoice->endorsement_e_date)->format('d/m/Y') }} </span>
                @endif
            </div>
        </div>
        ...
        <div class="clearfix mb-1">
            <table class="w-full border border-black border-collapse font-size-table">
                ...
                <tr>
                    ...
                    @if(empty($invoice->total_premuim))
                        <td class="text-center">-</td>
                    @else
                        <td class="font-bold text-center">{{ '$'.number_format($invoice->total_premuim , 2) }}</td>
                    @endif
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    @if(empty($invoice->total_premuim) || empty($invoice->exch_rate))
                        <td class="text-center">-</td>
                    @else
                        <td class="font-bold text-center">{{ number_format(round($invoice->total_premuim * $invoice->exch_rate), 2) }} <span class='font-khmer-os'>រៀល</span></td>
                    @endif
                </tr>
                <tr>
                    <td class="padding-left-5 ">
                    ...
                    @if(empty($invoice->exch_rate))
                        <td class="text-center">-</td>
                    @else
                        <td class="text-center">{{ number_format($invoice->exch_rate, 2) }} <span class='font-khmer-os'>រៀល</span></td>
                    @endif
                </tr>
            </table>
        </div>
            ...
            <div class="w-1/3 inline-block float-right">
                <div class="relative" style="min-height: 100px;">
                    @if ($signature && file_exists(public_path($signature->file_url)))
                        <img class="img-over" alt="" src="{{ public_path($signature->file_url) }}" style="max-height: 100px">
                    @endif
                </div>
