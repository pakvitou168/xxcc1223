...
class ClaimPaidExport implements FromCollection, WithHeadings, ShouldAutoSize, WithMapping, WithColumnWidths, WithStyles, WithColumnFormatting, WithStrictNullComparison
{
    ...
    private function getClaimPaidReportData()
    {
        ...
        if ($this->from_date && $this->to_date) {
            $claimPaidReportData = ClaimReportV::whereBetween('payment_approved_at', [$this->from_date, Carbon::parse($this->to_date)->addDays(1)->format('Y-m-d')])
                ->get()
                ->groupBy(function ($item) {
                    return Carbon::parse($item->claim_date)->format('Y');
                })->toArray();
        } else if ($this->from_date && !$this->to_date) {
            $claimPaidReportData = ClaimReportV::whereDate('payment_approved_at', '>=', $this->from_date)
                ->get()->groupBy(function ($item) {
                        return Carbon::parse($item->claim_date)->format('Y');
                })->toArray();
        } else if (!$this->from_date && $this->to_date) {
            $claimPaidReportData = ClaimReportV::whereDate('payment_approved_at', '<=', $this->to_date)
                ->get()
                ->groupBy(function ($item) {
                    return Carbon::parse($item->claim_date)->format('Y');
                })->toArray();
