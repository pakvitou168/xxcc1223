...
use App\Models\Claim\ClaimIncurredReport;
use Carbon\Carbon;
...
class ClaimInCurredExport implements FromCollection, WithHeadings, ShouldAutoSize, WithMapping, WithColumnWidths, WithStyles, WithColumnFormatting, WithStrictNullComparison
{
    ...
    private function getClaimPaidReportData()
    {
        ...
        if ($this->from_date && $this->to_date) {
            $claimPaidReportData = ClaimIncurredReport::where('loss_of_deductible','<>',0)->whereBetween('approved_at', [$this->from_date, Carbon::parse($this->to_date)->addDays(1)->format('Y-m-d')])
            ...
            $claimPaidReportData = ClaimIncurredReport::where('loss_of_deductible','<>',0)->whereDate('approved_at', '>=', $this->from_date)->orderByDESC('status')->orderByDESC('claim_no')->get()->groupBy(function ($item) {
                if (isset($item->updated_at)) {
                    return $item->updated_at->format('Y');
                }
                return $item->created_at->format('Y');
            ...
            $claimPaidReportData = ClaimIncurredReport::where('loss_of_deductible','<>',0)->whereDate('approved_at', '<=', $this->to_date)->orderByDESC('status')->orderByDESC('claim_no')->get()->groupBy(function ($item) {
                if (isset($item->updated_at)) {
                    return $item->updated_at->format('Y');
                }
                return $item->created_at->format('Y');
            ...
            $claimPaidReportData = ClaimIncurredReport::where('loss_of_deductible','<>',0)->orderByDESC('status')->orderByDESC('claim_no')->get()->groupBy(function ($item) {
                if (isset($item->updated_at)) {
                    return $item->updated_at->format('Y');
                }
                return $item->created_at->format('Y');
    ...
    private function conditionalTotalRow($sheet, $endRow)
    {
        ...
        $sheet->setCellValue('N' . $endRow, $this->sumTotalCol('loss_of_deductible'));
