...
use App\Models\Insurance\PolicyReport\PolicyReport;

class PolicyExport implements FromCollection, WithHeadings, ShouldAutoSize, WithMapping, WithColumnWidths, WithStyles, WithColumnFormatting, WithStrictNullComparison
{
    ...
    */
    ...
    private function getVehicleReportData(){
        if($this->issue_date_from && $this->issue_date_to)
            return PolicyReport::whereDate('date_of_issue', '>=' ,$this->issue_date_from)->whereDate('date_of_issue', '<=',$this->issue_date_to)->orderByDesc('id')->get();
        else if($this->issue_date_from && !$this->issue_date_to)
            return PolicyReport::whereDate('date_of_issue', '>=', $this->issue_date_from)->orderByDesc('id')->get();
        else if(!$this->issue_date_from && $this->issue_date_to)
            return PolicyReport::whereDate('date_of_issue', '<=', $this->issue_date_to)->orderByDesc('id')->get();
        else
            return PolicyReport::orderByDesc('id')->get();
    }
