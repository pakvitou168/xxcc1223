...
use App\Models\Travel\Policy\Insurance\PolicyView;
...
class ReinsuranceExport implements FromCollection, WithHeadings, ShouldAutoSize, WithMapping, WithColumnWidths, WithStyles, WithColumnFormatting, WithStrictNullComparison
{
    ...
    private function getReinsuranceData($policyId)
    {
        ...
        foreach ($reinsuranceList as $index => $reinsurance) {
            if ($index == 0) {
                $reinsurance['no'] = $index + 1;
                $reinsuranceList[$index] = $reinsurance;
            }
            $reinsuranceExport = array_merge($reinsuranceExport, array($reinsuranceList[$index]));
            if (count($reinsuranceList[$index]['sub_reinsurance_data']) > 0) {
                foreach ($reinsuranceList[$index]['sub_reinsurance_data'] as $subInsuranceData) {
    ...
    {
        $policy = PolicyView::withoutGlobalScope(PolicyScope::class)->find($this->policyId);
