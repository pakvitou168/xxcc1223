...
use App\Models\Insurance\PolicyReport\AutoPolicyRegisterReport;
    ...
    */
    ...
    private function getVehicleReportData(){
        if($this->issue_date_from && $this->issue_date_to){
            return AutoPolicyRegisterReport::where('apv_status', 'APV')->whereBetween('issued_date', [$this->issue_date_from, \Carbon\Carbon::parse($this->issue_date_to)->addDays(1)->format('Y-m-d')])->get();
        }
        else if($this->issue_date_from && !$this->issue_date_to)
            return AutoPolicyRegisterReport::where('apv_status', 'APV')->whereDate('issued_date', '>=', $this->issue_date_from)->get();
        else if(!$this->issue_date_from && $this->issue_date_to)
            return AutoPolicyRegisterReport::where('apv_status', 'APV')->whereDate('issued_date', '<=', $this->issue_date_to)->get();
        else
            return AutoPolicyRegisterReport::where('apv_status', 'APV')->get();
    }
    ...
    private function sumTotalCol($column_name){
        return $this->vehicleReportData->sum($column_name);
    }
