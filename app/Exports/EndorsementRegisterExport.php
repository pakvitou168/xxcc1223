...
use App\Models\Insurance\PolicyReport\AutoEndorsementRegisterReport;

class EndorsementRegisterExport implements FromCollection, WithHeadings, ShouldAutoSize, WithMapping, WithColumnWidths, WithStyles, WithColumnFormatting, WithStrictNullComparison
{
    ...
    */
    ...
    private function getVehicleReportData(){
        if($this->issue_date_from && $this->issue_date_to)
            return AutoEndorsementRegisterReport::where('apv_status', 'APV')->whereBetween('issued_date', [$this->issue_date_from, \Carbon\Carbon::parse($this->issue_date_to)->addDays(1)->format('Y-m-d')])->get();
        else if($this->issue_date_from && !$this->issue_date_to)
            return AutoEndorsementRegisterReport::where('apv_status', 'APV')->whereDate('issued_date', '>=', $this->issue_date_from)->get();
        else if(!$this->issue_date_from && $this->issue_date_to)
            return AutoEndorsementRegisterReport::where('apv_status', 'APV')->whereDate('issued_date', '<=', $this->issue_date_to)->get();
        else
            return AutoEndorsementRegisterReport::where('apv_status', 'APV')->get();
    }
    ...
    public function styles(Worksheet $sheet) {
        ...
        // Global
        $sheet->getStyle('A:DM')->getAlignment()->setVertical(Alignment::VERTICAL_TOP);
    ...
    private function conditionalStyles($sheet, $startRow, $endRow) {
        $sheet->getStyle('B:DM')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        ...
        $sheet->getStyle('DM')->getAlignment()->setWrapText(true);
        $sheet->getStyle('A'.$startRow.':DM'.$endRow)->getFont()->setSize(10);
        $sheet->getStyle('A'.$startRow.':DM'.$endRow)->applyFromArray([
            'borders' => [
                'allBorders' => [
    ...
    private function conditionalTotalRow($sheet, $endRow){
        ...
        $sheet->setCellValue('BV'.$endRow, $this->sumTotalCol('quota_share_ri_commission_amount'));
        $sheet->setCellValue('BW'.$endRow, $this->sumTotalCol('quota_share_net_due'));

        // PGI Retention
        $sheet->setCellValue('BY'.$endRow, $this->sumTotalCol('pgi_retention_sum_insured_ceded'));
        $sheet->setCellValue('BZ'.$endRow, $this->sumTotalCol('pgi_retention_premium_ceded'));
        $sheet->setCellValue('CA'.$endRow, $this->sumTotalCol('pgi_retention_tax_fee_amount'));
        $sheet->setCellValue('CB'.$endRow, $this->sumTotalCol('pgi_retention_net_due'));

        // OTHER TREATY CESSIONS
        $sheet->setCellValue('CE'.$endRow, $this->sumTotalCol('reinsure_other_sum_insured_ceded'));
        $sheet->setCellValue('CF'.$endRow, $this->sumTotalCol('reinsure_other_premium_ceded'));
        $sheet->setCellValue('CG'.$endRow, $this->sumTotalCol('reinsure_other_tax_fee_amt'));
        $sheet->setCellValue('CI'.$endRow, $this->sumTotalCol('reinsure_other_ri_commission_amount'));
        $sheet->setCellValue('CJ'.$endRow, $this->sumTotalCol('reinsure_other_net_due'));

        // OUTWARD FACULTATIVE REINSURANCE
        $sheet->setCellValue('CM'.$endRow, $this->sumTotalCol('facul_sum_insured_ceded'));
        $sheet->setCellValue('CN'.$endRow, $this->sumTotalCol('facul_premium_ceded'));
        $sheet->setCellValue('CO'.$endRow, $this->sumTotalCol('facul_tax_fee_amounts'));
        $sheet->setCellValue('CQ'.$endRow, $this->sumTotalCol('facul_ri_commission_amount'));
        $sheet->setCellValue('CR'.$endRow, $this->sumTotalCol('facul_net_due'));

        // OUTWARD CO-INSURANCE
        $sheet->setCellValue('CU'.$endRow, $this->sumTotalCol('outward_sum_insured_ceded'));
        $sheet->setCellValue('CV'.$endRow, $this->sumTotalCol('outward_premium_ceded'));
        $sheet->setCellValue('CX'.$endRow, $this->sumTotalCol('outward_ri_commission_amount'));
        $sheet->setCellValue('CY'.$endRow, $this->sumTotalCol('outward_net_due'));

        //
        $sheet->setCellValue('CZ'.$endRow, $this->sumTotalCol('reinsurance_premium_ceded'));
    ...
    private function conditionalMergeTitle($sheet, $startRow){
        ...
        $sheet->mergeCells('BQ'.$startRow.':BW'.$startRow);
        ...
        $sheet->setCellValue('BX'.$startRow, 'PGI RETENTION');
        $sheet->mergeCells('BX'.$startRow.':CB'.$startRow);
        ...
        $sheet->setCellValue('CI'.($startRow +1), $sheet->getCell('CI'.$startRow));
        $sheet->setCellValue('CJ'.($startRow +1), $sheet->getCell('CJ'.$startRow));
        $sheet->setCellValue('CC'.$startRow, 'OTHER TREATY CESSIONS');
        $sheet->mergeCells('CC'.$startRow.':CJ'.$startRow);
        ...
        $sheet->setCellValue('CK'.$startRow, 'OUTWARD FACULTATIVE REINSURANCE');
        $sheet->mergeCells('CK'.$startRow.':CR'.$startRow);
        ...
        $sheet->setCellValue('CS'.$startRow, 'OUTWARD CO-INSURANCE');
        $sheet->mergeCells('CS'.$startRow.':CY'.$startRow);

        //
        $sheet->mergeCells('CZ'.$startRow.':CZ'.($startRow +1));
        $sheet->mergeCells('DA'.$startRow.':DA'.($startRow +1));
        $sheet->mergeCells('DB'.$startRow.':DB'.($startRow +1));
        $sheet->mergeCells('DC'.$startRow.':DC'.($startRow +1));
    ...
    private function conditionalStyleTitle($sheet, $startRow){
        $sheet->getStyle('A'.($startRow - 1).':DM'.($startRow - 1))->getFont()->setSize(10)->setBold(true);
        $sheet->getStyle('A'.$startRow.':DM'.$startRow)->getFont()->setSize(10)->setBold(true);
        $sheet->getStyle('A'.($startRow + 1).':DM'.($startRow + 1))->getFont()->setSize(10)->setBold(true);
        $sheet->getStyle('A'.$startRow.':DM'.$startRow)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->getStyle('A'.$startRow.':DM'.$startRow)->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);

        $sheet->getStyle('A'.($startRow+1).':DM'.($startRow+1))->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        $sheet->getStyle('A'.($startRow+1).':DM'.($startRow+1))->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
    ...
    private function conditionalColorTitle($sheet, $startRow){
        ...
        $sheet->getStyle('A'.$startRow.':DM'.($startRow+1))->getFill()->applyFromArray([
            'fillType' => 'solid',
            'rotation' => 0,
            'color' => ['rgb' => 'FFC000'],
        ]);
    ...
    private function sumTotalCol($column_name){
        return
            $this->vehicleReportData->sum($column_name) < 0 ?
                '('. number_format(abs($this->vehicleReportData->sum($column_name)), 2,'.', ',') .')' :
                $this->vehicleReportData->sum($column_name);
    }
